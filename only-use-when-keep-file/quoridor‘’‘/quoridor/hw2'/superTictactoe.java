...
public class superTictactoe extends Tictactoe {
    ...
    public void startplay(){
        ...
        while(!this.finish) {
            ...
            while(choose_b==-1){
                while(!set_b){
                    System.out.println(this.player_now.get_name()+",enter which board");
                ...
                choose_b=supercell_now.play_in_a_iboard(player_now);
                if (choose_b==-1){//rechoose big board
                    System.out.println(this.s_b);
                    set_b=false;
                }else if (choose_b==-2){//quit game
                    System.out.println("player " + this.player_now.get_name() + " give up");
            ...
            if (supercell_now.get_win()) {
                this.board.set_p((int) Math.floor((sb_num - 1) / this.o_M), ((sb_num- 1) % this.o_M) , player_now.get_piece());
                this.CheckWin((int) Math.floor((sb_num - 1) / this.o_M), ((sb_num - 1) % this.o_M), player_now.get_piece());
            }
            if (this.finish) {
                System.out.println("player " + this.player_now.get_name() + " win");
