...
public class Orderandchaosgame extends Generalboardgame {
	...
	public void StartPlay() {
		...
		while(!this.finish) {
			...
			System.out.println(player_now.get_name()+",pick a pieces,0 for "+p1.getPiece_Ptest()+"," +
					"1 for "+p2.getPiece_Ptest());
			int p=this.CheckSetUp(0,1);
			if(p==0){
				player_now.set_pieces(p1);
			}else {
				player_now.set_pieces(p2);
			}
			...
			int row=coord[0];
			int col=coord[1];
			this.board.set_p(row,col,this.player_now.get_piece());
			this.board.print_with_location();
